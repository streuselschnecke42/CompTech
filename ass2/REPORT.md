...
The program can be visible by using minicom. When loading the file onto the pico with "sudo picotool load HelloWorld.uf2" and then force-restart the pico with "sudo picotool reboot -f", it will then remove the pico as a data drive in your device and execute the program on the pico. I used the command "minicom -b 115200 -o -D /dev/ttyACM0" to open minicom and see the program's output.\
